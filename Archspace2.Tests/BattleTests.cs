...
namespace Archspace2.Battle
{
    ...
    public class BattleTests
    {
        ...
        public void FleetShouldTurn()
        {
            ...
            fleetA1.Deploy(2500, 5000, 0, Command.Formation);
            fleetA2.Deploy(2500, 5020, 0, Command.Formation);
            fleetA3.Deploy(2500, 4980, 0, Command.Formation);
            ...
            fleetD1.Deploy(7500, 5000, 180, Command.Formation);
            fleetD2.Deploy(7500, 5020, 180, Command.Formation);
            fleetD3.Deploy(7500, 3000, 180, Command.Formation);

            Battle simulation = new Battle(BattleType.Siege, attacker, defender, battlefield, attackingArmada, defendingArmada);

            bool triedToTurnToCentre = false;

            while (!simulation.IsComplete)
            {
                simulation.RunTurn();
                foreach (Fleet fleet in simulation.AttackingFleets.Union(simulation.DefendingFleets))
                {
                    if (fleet.Substatus == FleetSubstatus.TurningToCentre)
                    {
                        triedToTurnToCentre = true;
                    }
                }
            }

            Assert.IsTrue(triedToTurnToCentre);
            Assert.IsTrue(simulation.IsComplete);
            Assert.IsTrue(simulation.Record.BattleOccurred);
            Assert.IsTrue(simulation.Record.Events.Any());
            Assert.IsTrue(simulation.Record.Events.Any(x => x.Type == RecordEventType.Movement), "No fleets moved.");
